...
// Diccionario heurístico rápido de artistas y palabras clave comunes en bares
const GENRE_RULES = [
  {
    genre: 'Salsa',
    keywords: ['salsa', 'guaguanco', 'son montuno', 'timba', 'descarga', 'boogaloo', 'salsa brava', 'salsa choque', 'salsa romantica'],
    artists: [
      'grupo niche', 'joe arroyo', 'hector lavoe', 'willie colon', 'ruben blades', 'cheo feliciano',
      'oscar d\'leon', 'los van van', 'gran combo', 'guayacan', 'titanes', 'frankie ruiz',
      'eddie santiago', 'miky taveras', 'adolescentes orquesta', 'puerto rican power', 'gilberto santa rosa',
      'victor manuelle', 'orquesta guayacan', 'orquesta la 33', 'rey ruiz', 'tony vega', 'tito nieves',
      'ismael rivera', 'ismael miranda', 'la sonora ponceña', 'celia cruz', 'papo lucca', 'ray barreto',
      'charry', 'yiyo sarante', 'michele batista', 'chiquito team band', 'david pabon', 'maelo ruiz'
    ]
  },
  {
    genre: 'Popular',
    keywords: ['musica popular', 'música popular', 'despecho', 'cantina', 'guaro', 'aguardiente', 'traicion', 'charrascal', 'ranchera', 'norteña', 'corrido', 'regional'],
    artists: [
      'dario gomez', 'luis alberto posada', 'charrito negro', 'jhonny rivera', 'jessi uribe', 'pipe bueno',
      'paola jara', 'alzate', 'arelys henao', 'yeison jimenez', 'francy', 'carin leon', 'christian nodal',
      'hernan gomez', 'el andariego', 'alexis escobar', 'hebert vargas', 'luisito muñoz', 'sebastian campos',
      'los tigres del norte', 'calibre 50', 'fidel rueda', 'los relicarios', 'las hermanitas calle',
      'antonio aguilar', 'vicente fernandez', 'pedro infante', 'javier solis', 'alejandro fernandez'
    ]
  },
  {
    genre: 'Vallenato',
    keywords: ['vallenato', 'acordeon', 'paseo', 'merengue vallenato', 'puya', 'son vallenato', 'parranda'],
    artists: [
  ...
  {
    genre: 'Reggaetón',
    keywords: ['reggaeton', 'reggaetón', 'regaeton', 'regeton', 'urbano', 'trap', 'dembow', 'perreo'],
    artists: [
      'feid', 'bad bunny', 'karol g', 'daddy yankee', 'don omar', 'j balvin', 'maluma', 'wiskin y yandel',
      'wisin & yandel', 'plan b', 'arcangel', 'nicky jam', 'zion y lennox', 'chencho corleone', 'rauw alejandro',
      'ryan castro', 'blessd', 'anuel', 'ozuna', 'ivy queen', 'tegocalderon', 'tego calderon', 'farruko'
    ]
  },
  {
    genre: 'Merengue / Bachata',
    keywords: ['merengue', 'bachata'],
    artists: [
      ...
      'anthony santos', 'los toros band', 'rikarena', 'elvis crespo', 'zacarias ferreira'
    ]
  },
  {
    genre: 'Rock / Pop',
    keywords: ['rock en español', 'pop', 'ska', 'rock', 'clasicos del rock'],
    artists: [
      ...
      'la mosca', 'los fabulosos cadillacs', 'autenticos decadentes', 'mana', 'juanes', 'kraken', 'aterciopelados'
    ]
  }
];
...
class AIDJEngine {
   ...
   */
  async classifyGenre(title, artist) {
    const fullText = `${title || ''} ${artist || ''}`.toLowerCase();

    // 1. Detección heurística inmediata
    for (const rule of GENRE_RULES) {
      // Coincidencia con nombres de artistas
...
Responde ÚNICAMENTE con el nombre del género de la lista anterior, sin puntuación ni texto adicional.`;
  ...
  calculateSmartSlot(queue, newSongGenre, currentlyPlaying = null) {
    ...
    // 1. Si la canción que está sonando actualmente es del mismo género
    // y la primera canción de la cola también es del mismo género,
    // buscamos el final de ese bloque actual para sumarla a la tanda en curso
    ...
    // Si encontramos una canción del mismo género dentro de la ventana de espera
    if (lastMatchingIndex !== -1) {
      // La insertamos justo después de la última del mismo género
      return lastMatchingIndex + 1;
    }

    // 2. Si la que está sonando actualmente es del mismo género pero en la cola aún no hay de ese género:
    if (currentlyPlaying && currentlyPlaying.genre === newSongGenre) {
      // La insertamos de primera en la cola para continuar la tanda actual
      return 0;
    }

    // 3. Si no hay canciones del mismo género en la ventana:
    // Insertamos al final de la ventana para permitir que el bloque actual termine
    // y esta canción abra la nueva tanda sin demorar demasiado al cliente.
    return searchLimit;
  }
